...
if (Hls.isSupported()) {
    ...
    hls.attachMedia(video), hls.on(Hls.Events.MEDIA_ATTACHED, function() {
        $.ajax({
            url: "http://www.umbali.live/get/live/".concat(s).concat("/"),
            method: "POST",
            data: d,
            success: function(e) {
...
} else if (video.canPlayType("application/vnd.apple.mpegurl")) $.ajax({
    url: "http://www.umbali.live/get/live/".concat(s).concat("/"),
    method: "POST",
    data: d,
    success: function(e) {
        video.src = e.hls, video.play(), video.addEventListener("loadedmetadata", function() {
            video.play()
        })
    },
    statusCode: {
